...
public class JSONFieldParser {
      ...
      for (int i = 0, l = jsonArray.length(); i < l; i++) {
        if (arrayClass.equals(String.class)) {
          ...
          arrayValue[i] = parseEntity(arrayClass, jsonArray.getJSONObject(i), skipNonExistingFields);
            ...
            Class<?> arrayClassNested = arrayClass.getComponentType();
            Object[] arrayValueNested = (Object[])Array.newInstance(arrayClassNested, jsonArrayNested.length());
            
            for (int j = 0, ll = jsonArrayNested.length(); j < ll; j++) {
              if (arrayClassNested.equals(String.class)) {
            	  arrayValueNested[j] = jsonArrayNested.getString(j);
              } else if (arrayClassNested.equals(Integer.class)) {
            	  arrayValueNested[j] = jsonArrayNested.getInt(j);  
              } else if (arrayClassNested.equals(Long.class)) {
            	  arrayValueNested[j] = jsonArrayNested.getLong(j);  
              } else if (arrayClassNested.equals(Double.class)) {
            	  arrayValueNested[j] = jsonArrayNested.getDouble(j);  
              } else if (arrayClassNested.equals(Boolean.class)) {
            	  arrayValueNested[j] = jsonArrayNested.getBoolean(j);
              } else if (isFoursquareEntity(arrayClassNested)) {
            	  arrayValueNested[j] = parseEntity(arrayClassNested, jsonArrayNested.getJSONObject(j), skipNonExistingFields);
              } else {
                throw new FoursquareApiException("Unknown array type: " + arrayClassNested);
              }
            }
            
            arrayValue[i] = arrayValueNested;
        } else {
  ...
  private static Field getField(Class<?> entityClass, String fieldName) {
    try {
      // Replace '-' with '_' when mapping JSON field name to Java field name since '-' is illegal in Java.
      Field field = entityClass.getDeclaredField(fieldName.replace('-', '_'));
  ...
  private static Method getSetterMethod(Class<?> entityClass, String fieldName) {
    ...
    methodNameBuilder.append(fieldName.substring(1).replace('-', '_'));
  ...
  private static void setEntityFieldValue(FoursquareEntity entity, String fieldName, Object value) throws FoursquareApiException {
    ...
    try {
      fieldProperty = getField(entity.getClass(), fieldName.replace('-', '_'));
