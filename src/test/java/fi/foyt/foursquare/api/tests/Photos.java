...
public class Photos {
  ...
  public final void testPhoto() throws FoursquareApiException {
    ...
    assertEquals("https://playfoursquare.s3.amazonaws.com/pix/UYU54GYOCURPAUYXH5V2U3EOM4DCX4VZPT3YOMN43H555KU2.jpg", result.getResult().getUrlOriginal());
    assertEquals(new Integer(540), result.getResult().getWidth());
    assertEquals(new Integer(720), result.getResult().getHeight());
  ...
  public final void testPhotosAdd() throws FoursquareApiException, IOException {
    ...
    assertEquals("https://playfoursquare.s3.amazonaws.com/pix/XZVRBK41YC4BEQULXHQWHBTR4RMH03RF3QBEPS5FQBACHFMU.jpg", result.getResult().getUrlOriginal());
    assertEquals(new Integer(450), result.getResult().getWidth());
    assertEquals(new Integer(664), result.getResult().getHeight());
